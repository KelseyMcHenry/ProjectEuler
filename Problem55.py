...
In fact, 10677 is the first number to be shown to require over fifty iterations before producing a palindrome: 4
...
from HelperFunctions import flip_number, is_palindromic


lychrel_number_count = 0
for n in range(1, 10001):
    iterations = 1
    ongoing_value = n + flip_number(n)
    while not is_palindromic(ongoing_value):
        ongoing_value = ongoing_value + flip_number(ongoing_value)
        iterations += 1
        if iterations >= 50:
            # print(n)
